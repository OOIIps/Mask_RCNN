    ...
    def to_json(self):
        ...
        with open('demo_json.json', 'w') as f:
            ...
            for key in self.__dict__:
                js[key] = serialize_json(self.__dict__[key])    
            
            for x in dir(self):
                if not x.startswith("__") and not callable(getattr(self, x)):
                    js[x] = serialize_json(getattr(self, x))
        
            json.dump(js, f)
            
    def from_json(self, jname, train=False):
        
        print ('Loading training config from JSON file: %s'%(jname))
        ...
        for key in js:
            if train:
                if key in \
                  ['BATCH_SIZE', 'IMAGES_PER_GPU', 'GPU_COUNT', 'STEPS_PER_EPOCH', 'MASK_LAYER']:
